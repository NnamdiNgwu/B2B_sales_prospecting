...
class B2BMessagePersonalizer:
    ...
    def __init__(self, config, llm_api_key):
        ...
        self.personalization_template = self._preprocess_template_string(
            ...
            Rewrite the "Original Message" to make it more relevant and engaging to the target company. Keep it professional, concise (under 150 words), and focused on exploring potential partnerships.
            
            Do not invent services that you offer. The goal is to open a conversation about how your company's solutions (e.g., software, consulting) might align with their business.
    ...
    def personalize_message(self, website_content: str, original_message: str) -> str:
        """
        Generates a personalized B2B message.
        """
        logger.debug("Personalizing B2B message with LLM.")
        prompt = ChatPromptTemplate.from_template(self.personalization_template)
        chain = prompt | self.llm | StrOutputParser()
        
        try:
            personalized_message = chain.invoke({
                "website_content": website_content,
                "original_message": original_message
            })
            logger.info("Successfully generated personalized message.")
...
# You can either delete it or keep it for other purposes, but it should not be used here.
# class GPTAnswerer:
#    ...
# class GPTAnswerer:
